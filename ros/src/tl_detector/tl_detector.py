...
class TLDetector(object):
    ...
    def image_cb(self, msg):
        ...
        '''
        if self.state != state:
            rospy.loginfo('State changed: {}'.format(state))
            self.state_count = 0
            self.state = state
    ...
    def process_traffic_lights(self):
        ...
        if(self.pose and self.waypoint_tree):
            ...
            for idx, light in enumerate(self.lights):
                # get stop line for waypoint index
                line = stop_line_positions[idx]
                temp_wp_idx = self.get_closest_waypoint(line[0], line[1])

                # find closest stop line waypoint index 
                d = temp_wp_idx - car_wp_idx
                if d >=0 and d < diff: # d > 0 indicates the way point is ahead of the  car, d < diff indicates that the traffic light is closer 
                    diff = d
                    closest_light = light
